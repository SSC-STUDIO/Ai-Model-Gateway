...
const outDir = path.resolve('output/playwright/ui-review-live-20260509')
...
async function main() {
  ...
  for (const viewport of viewports) {
    ...
    if (await page.locator('.login-panel').first().isVisible().catch(() => false)) {
      ...
      await page.screenshot({ path: path.join(outDir, `${viewport.name}-login.png`), fullPage: true, timeout: 60000 })
    ...
    for (const route of routes) {
      ...
      await page.screenshot({ path: path.join(outDir, `${viewport.name}-${route.name}.png`), fullPage: true, timeout: 60000 })
      ...
      for (const subClick of route.subClicks || []) {
        ...
        await page.screenshot({ path: path.join(outDir, `${viewport.name}-${route.name}-${sub}.png`), fullPage: true, timeout: 60000 })
